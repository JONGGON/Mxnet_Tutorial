...
class LogisticRegression(mx.operator.CustomOp):
...
class LogisticRegressionProp(mx.operator.CustomOpProp):
    ...
    def __init__(self,grad_scale):
        ...
        '''
        super(LogisticRegressionProp, self).__init__(False)
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return LogisticRegression(self.grad_scale)
...
def NeuralNet(epoch,batch_size,save_period,load_weights):
    ...
    output = mx.sym.Custom(data= output_affine , label = label , grad_scale = 1 , name="LogisticRegression", op_type = 'LogisticRegression') #
