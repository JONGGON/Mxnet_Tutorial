...
def algorithm(content_a=1, style_b=1, content_image=None, style_image=None, noise_image=None ,image_size=(256,512), ctx=mx.gpu(0)):
    ...
    #content loss
    content_loss=mx.sym.square(n-c)
    content_loss=0.5*content_loss.mean()

    style_loss=0
    #(5) compute style loss using cov1_1 ,cov2_1 ,cov3_1 ,cov4_1 ,cov5_1
    for n,s,shape in zip(group[7:],group[1:6],output_shapes[7:]): # or output_shapes[1:6]

        batch_size, filter, height, width = shape
        # reshape
