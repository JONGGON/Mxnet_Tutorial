...
def NeuralNet(epoch,batch_size,save_period,load_weights):
    ...
    for i in range(layer_number):
        if use_cudnn:
            cell.add(mx.rnn.FusedRNNCell(num_hidden=rnn_hidden_number, num_layers=1, bidirectional=False, mode="rnn_tanh", prefix="rnn_{}".format(i), params=None, forget_bias=1.0, get_next_state=True))
            if Dropout_rate > 0 and (layer_number-1) > i:
                cell.add(mx.rnn.DropoutCell(Dropout_rate, prefix="rnn_dropout_{}".format(i)))
        else:
            cell.add(mx.rnn.RNNCell(num_hidden=rnn_hidden_number, activation='tanh',prefix="rnn_{}".format(i)))
            if Dropout_rate > 0 and (layer_number - 1) > i:
                cell.add(mx.rnn.DropoutCell(Dropout_rate, prefix="rnn_dropout_{}".format(i)))
